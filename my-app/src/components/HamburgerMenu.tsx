...
const HamburgerMenu: React.FC<HamburgerMenuProps> = ({ desktopNav }) => {
  ...
  return (
    <>
      ...
      {/* 햄버거 버튼: 메뉴가 닫혀있을 때만 보임 */}
      {!isOpen && (
        <MenuButton isOpen={isOpen} onClick={() => setIsOpen(true)} aria-label="메뉴 열기">
          <div />
          <div />
          <div />
        </MenuButton>
      ...
      {/* 모바일 메뉴: 메뉴가 열려있을 때만 보임 */}
      <MobileMenuContainer ref={menuRef} isOpen={isOpen}>
        {/* X(닫기) 버튼: 메뉴가 열려있을 때만 보임 */}
        {isOpen && (
          <CloseButton aria-label="메뉴 닫기" onClick={() => setIsOpen(false)}>
            &times;
          </CloseButton>
        )}
